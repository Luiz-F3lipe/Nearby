...
class HomeView: UIView {
    ...
    func configureTableViewDelegate(_ delegate: UITableViewDelegate, dataSouce: UITableViewDataSource) {
        placesTableView.delegate = delegate
        placesTableView.dataSource = dataSouce
    }
    
    func setupPanGesture() {
        let panGeture = UIPanGestureRecognizer(target: self, action: #selector(handlePanGesture(_:)))
        containerView.addGestureRecognizer(panGeture)
    }
